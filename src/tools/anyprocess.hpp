...
struct err {
    ...
    [[noreturn]] GernalType operator()(const std::any &v) const {
        error(std::string("Unknown held type of any when visit: ") + v.type().name());
    }
};
...
inline auto visit(Func &&func, Any &&v) {
    if (v.type() == typeid(double)) {
        ...
        if constexpr (std::is_constructible_v<decltype(func(*std::any_cast<double>(&v))),
            ...
            return static_cast<decltype(func(*std::any_cast<double>(&v)))>(std::invoke(FuncOnErr{}, v));
        } else {
            std::invoke(FuncOnErr{}, v);
